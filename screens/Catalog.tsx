...
export function Catalog({ navigation }: any) {
  ...
  const init = async () => {
    ...
    const generoOk =
      generoSelecionado === ''
        ? true
        : manga.generos.some(
            g => g.nome_genero === generoSelecionado
          );
  ...
  const generosFiltrados = generosDisponiveis.filter(g =>
    g.nome_genero
      .toLowerCase()
      .includes(buscaGenero.toLowerCase())
  );
    ...
    <FlashList
      ...
      renderItem={({ item }) => (
        <TouchableOpacity
          style={[
            styles.generoBotao,
            generoSelecionado === item.nome_genero &&
            styles.generoSelecionado
          ]}
          onPress={() =>
